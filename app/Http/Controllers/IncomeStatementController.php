...
use App\Exports\Excel\IncomeStatementExcelExport;
use App\Exports\Pdf\IncomeStatementPdfExport;
...
class IncomeStatementController extends Controller
{
    ...
    private function getStatementData(Request $request): array
    {
        $fromDate = $request->query('from_date');
        $toDate = $request->query('to_date');
        $currencyParam = $request->query('currency');

        Log::info("Income Statement Data: from=$fromDate, to=$toDate, currency=$currencyParam");
    ...
    public function exportExcel(Request $request)
    {
        try {
            $data = $this->getStatementData($request);
            $export = new IncomeStatementExcelExport();
            return $export->download($data, $request);
        } catch (\Exception $e) {
            Log::error("Income Statement Excel Export Error: " . $e->getMessage());
            return response()->json([
                'success' => false,
    ...
    public function exportPdf(Request $request)
    {
        try {
            $data = $this->getStatementData($request);
            $export = new IncomeStatementPdfExport();
            return $export->download($data, $request);
        } catch (\Exception $e) {
            Log::error("Income Statement PDF Export Error: " . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Failed to export PDF Income Statement',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
